                                    ...
                                    <div onClick={(e)=>{
                                        ...
                                        <div style={{display:'flex',flexDirection:'column',height:'60px',minWidth:'0'}}>    
                                            ...
                                            <div style={{height:'30px',alignItems:'center'}}>
                                                <span style={{fontWeight:'normal',paddingLeft:'5px',color:status[indices.indexOf(value[0])]==="(Typing...)"?bgr==='black'?'lime':'darkgreen':bgr==='black'?'white':'#222',fontSize:'17px'}}><span style={{color:`${value[1][value[1].length-1].startsWith('✔✔✔✔')?'deepskyblue':'darksalmon'}`}}>{status[indices.indexOf(value[0])]==="(Typing...)"?"":value[1][value[1].length-1].startsWith('✔✔')?'✔✔':value[1][value[1].length-1].startsWith('✔')?"✔":''}</span>{status[indices.indexOf(value[0])]==="(Typing...)"?"Typing...": value[1][value[1].length-1].slice(value[1][value[1].length-1].indexOf(' '),value[1][value[1].length-1].lastIndexOf(' '))}</span>
                        ...
                        if (index < info.length / 2) {
                            ...
                            return (
                                <div className='userinfo' key={index} style={{display:search_filter[index]}}> 
                                    <div style={{display:search_filter[index],flexDirection:'column',justifySelf:'center',alignSelf:'center',alignItems:'center',justifyContent:'space-evenly',width:'260px',height:'220px',backgroundColor:indices[index]===up_user?'forestgreen':'darkgreen',borderRadius:'20px',padding:'5px',marginBottom:info.length/2===index+1?'10px':'0px'}}>
