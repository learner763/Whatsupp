    ...
    useEffect(()=>
    {
        setmessages(prev=>
        {
            ...
            for(let i=0;i<previous.length;i++)
            {
                ...
                {
                    if(dates[k+1]-dates[k]==1)
                    {
                        previous[i][1].splice(dates[k],1)
                    }
                    
                }
                if(dates[dates.length-1]===previous[i][1].length-1)
                {
                    previous[i][1].splice(dates[dates.length-1],1)
                }
            }
            return previous
        })
    ...
    return(
        ...
        <div className='home' style={{display:`${loaded==true? 'flex':'none'}`}}>
                                ...
                                return(
                                    <div onClick={()=>{set_seen();set_disp_chat('none');setdisp('flex');update_receiver(value[0]);console.log(receiver);}} className='chat_bar' key={index} style={{display:'flex',flexDirection:'column'}} >
                                        ...
                                        <div style={{height:'35px'}}>
                                            <span style={{fontWeight:'normal'}}><span style={{color:`${value[1][value[1].length-1].startsWith('✔✔✔✔')?'blue':'lightgreen'}`}}>{status[indices.indexOf(value[0])]==="(Typing...)"?"":value[1][value[1].length-1].startsWith('✔✔')?'✔✔':value[1][value[1].length-1].startsWith('✔')?"✔":''}</span>{status[indices.indexOf(value[0])]==="(Typing...)"?"Typing...": value[1][value[1].length-1].slice(value[1][value[1].length-1].indexOf(' '),value[1][value[1].length-1].lastIndexOf(' '))}</span>
            ...
            <div className='msg_div' style={{display:disp}}>
                <button  style={{display:edit_icon,fontSize:'10px',borderRadius:'5px',cursor:'pointer'}} onClick={()=>{set_edit('none');document.getElementById('message').value='';set_msg_value('')}}>✏️❌</button>
                <button  style={{display:reply_icon,fontSize:'10px',borderRadius:'5px',cursor:'pointer'}} onClick={()=>{set_reply('none');set_reply_to('')}}>💬❌</button>
